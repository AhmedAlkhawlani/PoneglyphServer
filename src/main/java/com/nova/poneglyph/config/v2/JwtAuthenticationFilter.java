...
//                    unauthorizedJson(request,response, HttpServletResponse.SC_UNAUTHORIZED, "token_expired", "Access token expired. Use refresh token.");
//                    return;
//                } else {
//                    log.debug("Invalid access token for user {} on {}", userId, request.getRequestURI());
//                    unauthorizedJson(request,response, HttpServletResponse.SC_UNAUTHORIZED, "invalid_token", "JWT invalid.");
//                    return;
//                }
//            }
//
...
////    }
//    private void unauthorizedJson(HttpServletRequest request, HttpServletResponse response,
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    /**
     * Fixed: requires HttpServletRequest so we can read the request URI safely.
     */
    private void unauthorizedJson(HttpServletRequest request, HttpServletResponse response,
                                  int status, String error, String message) throws IOException {
        response.setStatus(status);
        ...
        String body = new com.fasterxml.jackson.databind.ObjectMapper().writeValueAsString(errorResponse);
        response.getWriter().write(body);
    }

    // minimal JSON escape for simple messages (kept)
    private String escapeJson(String s) {
        if (s == null) return null;
        StringBuilder sb = new StringBuilder();
        for (char c : s.toCharArray()) {
